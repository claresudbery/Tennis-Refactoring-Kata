using System;
using System.Collections;
using System.Collections.Generic;
using Xunit;

namespace Tennis.Tests
{
    public class TestDataGenerator : IEnumerable<object[]>
    {
        public static string Player1Name = "player1";
        public static string Player2Name = "player2";

        private readonly List<object[]> _data = new List<object[]>
        {
            new object[] {0, 0, "Love-All"},
            new object[] {1, 1, "Fifteen-All"},
            new object[] {2, 2, "Thirty-All"},
            new object[] {3, 3, "Deuce"},
            new object[] {4, 4, "Deuce"},
            new object[] {1, 0, "Fifteen-Love"},
            new object[] {0, 1, "Love-Fifteen"},
            new object[] {2, 0, "Thirty-Love"},
            new object[] {0, 2, "Love-Thirty"},
            new object[] {3, 0, "Forty-Love"},
            new object[] {0, 3, "Love-Forty"},
            new object[] {4, 0, $"Win for {TestDataGenerator.Player1Name}"},
            new object[] {0, 4, $"Win for {TestDataGenerator.Player2Name}"},
            new object[] {2, 1, "Thirty-Fifteen"},
            new object[] {1, 2, "Fifteen-Thirty"},
            new object[] {3, 1, "Forty-Fifteen"},
            new object[] {1, 3, "Fifteen-Forty"},
            new object[] {4, 1, $"Win for {TestDataGenerator.Player1Name}"},
            new object[] {1, 4, $"Win for {TestDataGenerator.Player2Name}"},
            new object[] {3, 2, "Forty-Thirty"},
            new object[] {2, 3, "Thirty-Forty"},
            new object[] {4, 2, $"Win for {TestDataGenerator.Player1Name}"},
            new object[] {2, 4, $"Win for {TestDataGenerator.Player2Name}"},
            new object[] {4, 3, $"Advantage {TestDataGenerator.Player1Name}"},
            new object[] {3, 4, $"Advantage {TestDataGenerator.Player2Name}"},
            new object[] {5, 4, $"Advantage {TestDataGenerator.Player1Name}"},
            new object[] {4, 5, $"Advantage {TestDataGenerator.Player2Name}"},
            new object[] {15, 14,   $"Advantage {TestDataGenerator.Player1Name}"},
            new object[] {14, 15,   $"Advantage {TestDataGenerator.Player2Name}"},
            new object[] {6, 4,     $"Win for {TestDataGenerator.Player1Name}"},
            new object[] {4, 6,     $"Win for {TestDataGenerator.Player2Name}"},
            new object[] {16, 14,   $"Win for {TestDataGenerator.Player1Name}"},
            new object[] {14, 16,   $"Win for {TestDataGenerator.Player2Name}"},
        };

        public IEnumerator<object[]> GetEnumerator() => _data.GetEnumerator();

        IEnumerator IEnumerable.GetEnumerator() => GetEnumerator();
    }

    public class TennisTests
    {
        [Theory]
        [ClassData(typeof(TestDataGenerator))]
        public void Tennis1Test(int p1, int p2, string expected)
        {
            var game = new TennisGame1(TestDataGenerator.Player1Name, TestDataGenerator.Player2Name);
            CheckAllScores(game, p1, p2, expected);
        }

        [Theory]
        [ClassData(typeof(TestDataGenerator))]
        public void Tennis2Test(int p1, int p2, string expected)
        {
            var game = new TennisGame2(new string[]{TestDataGenerator.Player1Name, TestDataGenerator.Player2Name});
            CheckAllScores(game, p1, p2, expected);
        }

        [Theory]
        [ClassData(typeof(TestDataGenerator))]
        public void Tennis3Test(int p1, int p2, string expected)
        {
            var game = new TennisGame3(TestDataGenerator.Player1Name, TestDataGenerator.Player2Name);
            CheckAllScores(game, p1, p2, expected);
        }

        private void CheckAllScores(ITennisGame game, int player1Score, int player2Score, string expectedScore)
        {
            var highestScore = Math.Max(player1Score, player2Score);
            for (var i = 0; i < highestScore; i++)
            {
                if (i < player1Score)
                    game.WonPoint(TestDataGenerator.Player1Name);
                if (i < player2Score)
                    game.WonPoint(TestDataGenerator.Player2Name);
            }

            Assert.Equal(expectedScore, game.GetScore());
        }
    }
}